    ...
    <div className={`flex flex-col ${className}`}>
      <label
        className={`${
          errors[name]
            ...
            : readOnly
            ? "text-gray-500"
            : colors[baseColor].text
        } font-medium relative top-3 left-4 bg-white px-2 max-w-max rounded-lg`}
      >
        {text}
      </label>
      <div className="w-full flex items-center">
        <input
          className={`${readOnly ? "text-gray-500" : "text-black"} ${
            type === "password"
            ...
            errors[name]
              ...
              : readOnly
              ? "border-gray-500"
              : colors[baseColor].border
          } ${
            type === "password"
              ? "rounded-ss-2xl rounded-es-2xl"
              : "rounded-2xl"
          } p-4 focus:outline-none h-15 w-full`}
          type={
        ...
        {type === "password" && (
          <div
            className={`h-15 py-4 pr-4 border-t-2 border-r-2 border-b-2 rounded-ee-2xl rounded-se-2xl ${
              errors[name]
                ? "border-red-600"
                : readOnly
                ? "border-gray-500"
          ...
          >
            {showPassword ? (
              <EyeSlashIcon className="text-tuna" />
            ) : (
              <EyeIcon className="text-tuna" />
            )}
          </div>
        )}
      </div>
      {errors[name] && (
        <p className="text-red-600 pt-2">
          <ErrorIcon className="inline" /> {errors[name]}
        </p>
      )}
    </div>
  );

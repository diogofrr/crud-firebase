...
export default function Birthdays({ clients, status }: IBirthdaysProps) {
  ...
  useEffect(() => {
    if (status !== "loading") {
      const filter = clients.filter(
        (client) =>
          timestampToDate(client.birthday).getMonth() === new Date().getMonth()
      );
  ...
  return (
    <div className="bg-white w-full md:w-56 overflow-y-auto overflow-x-hidden max-h-80 h-auto min-h-[20rem] p-2 shadow-xl rounded-2xl mb-4 flex flex-col items-center py-6">
      <p className="text-tuna font-medium text-lg mb-4">
        &#127874; Aniversariantes{" "}
        {filteredClients.length !== 0 && `(${filteredClients.length})`}
      </p>
      {filteredClients.length !== 0 ? (
        filteredClients.map((client) => {
          let clientNameString;
          const clientNameArray = client.name.split(" ");
          const clientBirthday = formatDateToDDMMYYYY(
            timestampToDate(client.birthday)
          ).split("/");

          if (clientNameArray.length > 1) {
            clientNameString = `${clientNameArray[0]} ${
              clientNameArray[clientNameArray.length - 1][0]
            }.`;
          } else {
            clientNameString = clientNameArray[0];
          }
          ...
          return (
            <Item
              key={client.id}
            >{`${clientNameString} - ${clientBirthday[0]}/${clientBirthday[1]}`}</Item>
          );
        })
      ) : (
